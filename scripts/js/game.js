...
$(document).ready(function(){
    // set variables
    const player = $("#player");
    let player_position_left = ($(window).width() - $("#player").width()) / 2;
    let player_position_top = ($(window).height() - $("#player").height()) / 2;
    // positioning player
    // mouse
    $(document).mousemove(function(event) {
        player_centre();
        let mouse_position_top = event.pageX;
        let mouse_position_left = event.pageY;
